...
Vamos a realizar un gasto de la cuenta monetaria de la ACOS3. La instrucción es DEBIT.
 Tiene su correspondiente APDU. Hay que efectuarlo usando la instruccion de debit; 
 no escribiendo directamente en los records del fichero Account File, pues se trata de simular
  el gasto que se efectuaría por un usuario con el método de gasto habitual: por medio de un terminal o de una máquina expendedora...etc.

Es conveniente ejecutar primero el script de clase: 
09-pide-INQUIRE-ACCOUNT-ACOS3.js; para ver si la tarjeta tiene saldo. De todas formas,
 si no tiene saldo, la cuenta indicará un error que hay que intepretar segun las tablas del manual.


En el script que realice el DEBIT incluir delante el script INQUIRE ACCOUNT y 
extraer de su respuesta el parámetro ATREF u otros si hiciera falta. No se admite 
ir a leer directamente al fichero de la cuenta (FF=05H), pues en un uso normal de la tarjeta
 eso no se permitiría.
...
pues la larjeta lo espera incrementado en una unidad. Si no se hace así no validará el DEBIT.


Vamos a gastar 15 euros
No hace falta usar Secure Messaging.
No hace falta que pida PIN.

Subir al servidor web el script que realiza el DEBIT de la Account.
*/
...
//print("Send_debit: "+send_credit);

//mac_new=new ByteString("E6"+mount+ttrefc+atref.add(1)+"00 00",HEX);
mac_new=new ByteString("E6"+mount+ttrefd+atref.add(1)+"00 00",HEX);
//11 bytes
print("new mac cal: "+mac_new);
mac_new=macdes3(mac_new, desKD)
print("new mac cal: "+mac_new[1]);
send_debit=mac_new[1]+" "+mount+" "+ttrefc
print("ttrefd: "+ttrefc)
//send_credit=MAC+":"+ttrefd.add(1)+":"+mount
//send_credit=desMAC+" "+mount+" "+ttrefd.add(1)
print("Send_debit: "+send_debit);
resp = card.plainApdu(new ByteString("80 E6 00 00 0B "+send_debit, HEX));
print("Codigo SW: " + card.SW.toString(16));
