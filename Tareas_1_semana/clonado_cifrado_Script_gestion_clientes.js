...
Se lee el número de serie de la tarjeta (se completa a 8 bytes si no
ocupa ocho bytes, rellenando a ceros por la derecha).
Se cifra con simple DES y con una clave C1C2C3C4C5C6C7C8. 
Se escribe en la tarjeta desde la posición 0x40 y siguientes.*/
...
// Problemas al cambiar el pin no cambia la respuesta 9000?.
	...
	resp = card.plainApdu(new ByteString("00 B0 00 00 08", HEX));
	print(resp);
	print("Código SW: " + card.SW.toString(16));
	
	var deskey_serial = new Key();
	deskey_serial.setComponent(Key.DES, key);
	result_serial = crypto.encrypt(deskey_serial, Crypto.DES_ECB, resp);
	print("reg "+result_serial);
...
//envia el dato y se almacena desde la posicion 80
	...
	resp_serial = card.sendApdu(0x00, 0xD6, 0x00, 0x40,result_serial);
	print("Código SW: " + card.SW.toString(16));
